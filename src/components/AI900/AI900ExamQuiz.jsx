...
import styles from './AI900ExamQuiz.module.css'
...
function AI900ExamQuiz({ onGoHome, onGoBackToExam, numberOfQuestions = 45, autoShowExplanation = false, examMode = 'exam', scheduler }) {
  ...
  useEffect(() => {
    try {
      let availableQuestions = AI900Questions
      
      // Filter questions based on exam mode
      if (examMode === 'exam') {
        // Exam mode: only single-select questions (filter out multi-select)
        availableQuestions = AI900Questions.filter(q => !q.questionType || q.questionType !== 'multiple')
      }
  ...
  useEffect(() => {
    ...
    const savedSession = loadExamState('ai900', sessionId)
  ...
  const handleRecoveryDecline = () => {
    clearExamState('ai900', sessionId)
    setShowSessionRecovery(false)
  }
  ...
  if (showSessionRecovery && loadExamState('ai900', sessionId)) {
    const savedSession = loadExamState('ai900', sessionId)
    return (
      <SessionRecovery
